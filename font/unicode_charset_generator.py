...
# from ftp://unicode.org/Public/MAPPINGS/OBSOLETE/EASTASIA/KSC/KSX1001.TXT
ksx1001 = """
...
def charset_kor():
    ...
    for i in range(0x1100, 0x11FF+1-6):
        txt += unichr(i)
    txt += u'\n'
    """
    # vowels
    #for i in range(0x3130, 0x318F+1):
    for i in range(0x3130+1, 0x3164+1):
        txt += unichr(i)
    txt += u'\n\n'

def charset_kor_KSX1001():
    global txt
    txt += u'# hangul syllable from KS X 1001\n'
    for s in ksx1001.split('\n'):
        if (s == ""):
            continue
        l = s.strip().split('  ', 3)
        if ("SYLLABLE" not in l[2]):
            continue
        ss = int(l[1][2:], 16)
        txt += unichr(ss)
    txt += u'\n\n'
...
def charset_jap():
    ...
    for i in range(0x3000, 0x301F):
        txt += unichr(i)
    txt += u'\n'
    #for i in range(0x3041, 0x309F+1):
    for i in range(0x3041, 0x3094+1):
        txt += unichr(i)
    txt += u'\n'
    #for i in range(0x30A1, 0x30FF+1):
    for i in range(0x30A1, 0x30F4+1):
        txt += unichr(i)
    txt += u'\n'
    """ 
    # AINU part
    ...
    txt += u'\n'
    """
    for i in range(0x32D1, 0x32FE+1):
        txt += unichr(i)
    txt += u'\n'
...
# japanese with jis0208
def charset_jis0208():
    global txt
    txt += u'# jis0208\n'
    for s in jis0208.split('\n'):
        if (s == ""):
            continue
        l = s.strip().split('\t', 4)
        #if ("SYLLABLE" not in l[3]):
        #    continue
        ss = int(l[2][2:], 16)
        txt += unichr(ss)
    txt += u'\n\n'
